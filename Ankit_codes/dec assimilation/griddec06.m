...
clear all
res=[];
fid = fopen('dec06list.txt');% reading all files for the month of decil in an array called 'filename'
while ~feof(fid)
    C=fgetl(fid);
...
M = [aod_ave_dec(:,1) aod_ave_dec(:,2) aod_ave_dec(:,3) aod_ave_dec(:,9) countvec];
xlswrite('aoddec06.xlsx', M);